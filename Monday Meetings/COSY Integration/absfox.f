...
*     OF RADLEN. OUTPUT IS THE VECTOR V2.
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
      DOUBLE PRECISION IP,L0,LAMBAR,LAMMAX,LATDIS,MASS,MASSE,
     &                 MAXSTEP,MPL,NA,L
...
*     ********************************************
      ...
      LV(I) = CC(NBEG(IL)+I)
       DO 6 J=1,6
        V1(I,J) = CC(NBEG(IV1+J)+I)
6      CONTINUE
7     CONTINUE
*
      DO 18 I=0,NPART-1 !START PARTICLE LOOP
      X   = V1(I,1)
      PX  = V1(I,2)
      Y   = V1(I,3)
      PY  = V1(I,4)
...
10    CONTINUE
     ...
     &         0)) THEN
      ...
      CTHX = PZ/SQRT(PZ**2+PX**2)  !OLD ANGLES: PROCESSES ASSUME
      CTHY = PZ/SQRT(PZ**2+PY**2)  !STRAIGHT LINE TRAJECTORY.
      THO  = ACOS(PZ/P)            !WILL ROTATE BACK INTO LAB FRAME LATER.
C
...
15    CONTINUE
      ...
      IF ((DE.GE.EI-MASS).OR.(EI.EQ.MASS)) THEN
       ...
       GOTO 16
      ENDIF
      EF   = EI-DE                 !NEW TOTAL ENERGY
      GAMF = EF/MASS
      PN   = SQRT(EF**2-MASS**2)   !NEW TOTAL MOMENTUM
C
*     SCATTER
*     *******
      IF (RADLEN.LE.0) RADLEN = 100*716.4/     !CRUDE APPROXIMATION OF USER
     &   (RHO*1E-6*Z*(Z+1)*LOG(287/SQRT(Z)))   !DOES NOT KNOW RAD LEN
*
      STX = PX/SQRT(PX**2+PZ**2)               !OLD ANGLE SIN THETA X
      XD  = L*STX                              !X DETERMINISTIC (L=HYPOTENUSE)
      STY = PY/SQRT(PY**2+PZ**2)
      YD  = L*STY
*
     ...
     &        (LOG(L/RADLEN))**2))
*
      PZR = SCATDIST(TH0,PN)                   !PZ ROTATED - NOT IN LAB FRAME YET
      PTR = SQRT(PN**2-PZR**2)                 !TRANSVERSE P ROTATED
      PZN = PZR*COS(THO)-PTR*SIN(THO)          !NEW PZ (IN LAB FRAME)
      PT  = PZR*SIN(THO)+PTR*COS(THO)          !NEW PT (IN LAB FRAME)
*
      CALL RANDOM_NUMBER(R1)
      R1  = R1*2*PI
      PXN = PT*COS(R1)
      PYN = PT*SIN(R1)
      X    = X+LATDIS(L,PXN,PZN,TH0)*CTHX+XD
      PX   = PXN
      Y    = Y+LATDIS(L,PYN,PZN,TH0)*CTHY+YD
      PY   = PYN
      TOF  = TOF+DTOF(EI,PN,MASS,L+DZ)
      GAM2 = EF/MASS
!      IF FLAG.EQ.0 FLAG = DECAY(GAM,GAM2,TOF) !TURN OFF DECAY
      EI = EF                                  !FOR LOOPING PURPOSES
*
C     CHECK FLAGS AGAIN
      IF (FLAG.EQ.0) THEN
       IF (EI.LE.MASS) THEN
...
16    CONTINUE
      IF ((FLAG.EQ.3).OR.(FLAG.EQ.5)) THEN     !'PARTICLE MISSED ABSORBER' OR 'DRIFT' FLAG
       EF = EI
       X = X+L*PX/SQRT(EF**2-MASS**2)
       Y = Y+L*PY/SQRT(EF**2-MASS**2)
       TOF = TOF+L/(C*SQRT(EI**2-MASS**2)/EI)
      ...
      CC(NBEG(IV1+6)+I) = EF
      IF (SAV.NE.0) THEN
        WRITE(SAV,20) X,PX,Y,PY,TOF,EF,FLAG
      ENDIF
...
C     THAT IS, IF E IS TOO LOW THE STEP MAY NOT BE A 'LANDAU' STEP
...
*
      DO 25 WHILE (CUL.LT.L0)  !LOOP THROUGH LANDAU-EXCLUSIVE STEPS
       ...
       GAM = EF/MASS
       BETA = SQRT(1-1/GAM**2)
C       IF (DF.NE.0) DCP = GET_DCP(BETA*GAM,MP)  !DISABLED CRUDE APPROXIMATION FOR NOW
